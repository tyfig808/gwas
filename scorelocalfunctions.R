...
sig_sl=function(lind,POS, th){
	zones=c(0,0,0)	
	mist=lind # why do they label as list ??
	auxpos=POS
	while(max(mist)>=th){
	  M_loc=which.max(mist)
		if(length(which(mist[1:M_loc]==0))==0){    #the peak is at the beginning of the chrom
			...
			auxpos=POS[tmp:length(mist)]
			}else{	
				m_loc=max(which(mist[1:M_loc]==0))			
				max=max(mist)
				zones=rbind(zones, c(auxpos[m_loc+1],auxpos[M_loc],max))
				tmp=which(mist[M_loc:length(mist)]==0) #first 0 score after peak
				if (length(tmp)>0){
				  auxpos=auxpos[c(1:m_loc,(min(tmp)+M_loc):length(mist))]
				  mist=mist[c(1:m_loc, (min(tmp)+M_loc):length(mist))]
